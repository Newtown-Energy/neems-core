...
pub struct ExecuteArgs {
    /// Timestamp to use for script execution (ISO 8601 format:
    /// YYYY-MM-DDTHH:MM:SS)
    #[arg(short, long)]
...
fn execute_script(args: ExecuteArgs) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let script_content =
        args.script.as_deref().unwrap_or_else(|| ScriptExecutor::get_default_script());
...
fn parse_timestamp(timestamp_str: &str) -> Result<NaiveDateTime, Box<dyn std::error::Error>> {
    ...
    let formats = ["%Y-%m-%dT%H:%M:%S", "%Y-%m-%d %H:%M:%S", "%Y-%m-%dT%H:%M", "%Y-%m-%d %H:%M"];
    ...
    Err(format!(
        "Unable to parse timestamp '{}'. Use format: YYYY-MM-DDTHH:MM:SS or 'now'",
        timestamp_str
    )
    .into())
}
