...
async fn get_company_by_name(
    client: &Client,
    session_cookie: &rocket::http::Cookie<'static>,
    name: &str,
) -> Company {
    let response = client.get("/api/1/Companies").cookie(session_cookie.clone()).dispatch().await;
...
async fn test_create_company() {
    ...
    let new_comp = CompanyInput { name: "Test Company".to_string() };

    let response = client.post("/api/1/Companies").json(&new_comp).dispatch().await;
...
async fn test_list_companies() {
    ...
    let response = client.get("/api/1/Companies").cookie(session_cookie).dispatch().await;
    assert_eq!(response.status(), Status::Ok);

    let odata_response: serde_json::Value =
        response.into_json().await.expect("valid JSON response");
    let list: Vec<Company> =
        serde_json::from_value(odata_response["value"].clone()).expect("valid companies array");
...
async fn test_delete_company() {
    ...
    // Get a pre-created test company to delete (Removable LLC has no users
    // assigned)
    ...
    let delete_response =
        client.delete(&delete_url).cookie(session_cookie.clone()).dispatch().await;
    assert_eq!(delete_response.status(), Status::NoContent);

    // Verify company is deleted by trying to get all companies
    let list_response =
        client.get("/api/1/Companies").cookie(session_cookie.clone()).dispatch().await;
    assert_eq!(list_response.status(), Status::Ok);

    let odata_response: serde_json::Value =
        list_response.into_json().await.expect("valid JSON response");
    let list: Vec<Company> =
        serde_json::from_value(odata_response["value"].clone()).expect("valid companies array");
    assert!(
        !list.iter().any(|c| c.id == company_to_delete.id),
        "Deleted company should not appear in list"
    );
}
...
async fn test_delete_nonexistent_company() {
    ...
    let delete_response =
        client.delete("/api/1/Companies/99999").cookie(session_cookie).dispatch().await;
