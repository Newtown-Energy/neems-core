...
//! the `test-staging` feature.

use neems_api::{
    api::data::{DataSourcesResponse, ReadingsResponse},
    models::{CompanyInput, NewRole, UserInput},
    orm::{
        DbConn,
...
async fn test_list_data_sources_success() {
    ...
    let sources_response: DataSourcesResponse =
        response.into_json().await.expect("valid DataSourcesResponse JSON");
...
async fn test_list_data_sources_response_structure() {
    ...
    let response_json: serde_json::Value = response.into_json().await.expect("valid JSON response");
    ...
    let sources = response_json["sources"].as_array().expect("sources should be an array");
    ...
    for source in sources {
        ...
        assert!(
            source.get("interval_seconds").is_some(),
            "Source should have 'interval_seconds' field"
        );
...
async fn test_get_schema_success() {
    ...
    let schema = schema_response["schema"].as_str().expect("schema field should be a string");
...
/// Test the /api/1/$metadata/schema endpoint contains expected database
/// objects.
///
...
///
/// NOTE: This test is disabled because readings endpoints now require
/// authentication. See test_newtown_staff_access() for an authenticated
/// version.
#[tokio::test]
#[ignore]
async fn test_get_source_readings_latest() {
    ...
    let sources: DataSourcesResponse =
        sources_response.into_json().await.expect("valid DataSourcesResponse JSON");
    ...
    let readings_response: ReadingsResponse =
        response.into_json().await.expect("valid ReadingsResponse JSON");
...
async fn test_get_source_readings_time_window() {
    ...
    let url = format!(
        "/api/1/DataSources/{}/Readings?since=2024-01-01T00:00:00Z&until=2024-12-31T23:59:59Z",
        test_source_id
    );
    let response = client.get(&url).dispatch().await;
...
async fn test_get_source_readings_invalid_params() {
    ...
    let url = format!(
        "/api/1/DataSources/{}/Readings?latest=10&since=2024-01-01T00:00:00Z",
        test_source_id
    );
    let response = client.get(&url).dispatch().await;
...
///
/// This test verifies the multi-source readings endpoint with source_ids
/// parameter.
#[tokio::test]
#[ignore]
async fn test_get_multi_source_readings() {
    ...
    for reading in &readings_response.readings {
        assert!(
            requested_sources.contains(&reading.source_id),
...
///
/// This test verifies that the multi-source endpoint returns 400 without
/// source_ids.
#[tokio::test]
#[ignore]
...
///
/// This test verifies that the endpoint returns 404 if any source doesn't
/// exist.
#[tokio::test]
#[ignore]
...
///
/// This test validates the JSON structure and field types of the readings
/// response.
#[tokio::test]
#[ignore]
async fn test_readings_response_structure() {
    ...
    assert!(
        response_json.get("source_id").is_some(),
        "Response should have 'source_id' field"
    );
    assert!(
        response_json.get("total_count").is_some(),
        "Response should have 'total_count' field"
    );

    let readings = response_json["readings"].as_array().expect("readings should be an array");
    ...
    for reading in readings {
        ...
        assert!(
            reading.get("quality_flags").is_some(),
            "Reading should have 'quality_flags' field"
        );
    ...
    db_conn
        .run(|conn| {
            ...
            let newtown_energy = match get_company_by_name(
                conn,
                &CompanyInput { name: "Newtown Energy".to_string() },
            ) {
                Ok(Some(company)) => company,
                Ok(None) => insert_company(conn, "Newtown Energy".to_string(), None)
                    .expect("Failed to create Newtown Energy company"),
                Err(e) => panic!("Failed to query Newtown Energy: {:?}", e),
            };

            // Create test company
            let test_company = insert_company(conn, "Test Company".to_string(), None)
...
async fn test_readings_endpoints_require_authentication() {
    ...
    let response = client.get("/api/1/Readings?source_ids=1&latest=1").dispatch().await;
...
async fn test_company_based_access_control() {
    ...
    // Try to access a source (this might fail if the test source belongs to a
    // different company)
    let url = format!("/api/1/DataSources/{}/Readings?latest=1", test_source_id);
    let response = client.get(&url).cookie(session_cookie).dispatch().await;

    // Response should be either OK (if source belongs to test company) or Forbidden
    // (if not) The key is that it's not Unauthorized anymore
    assert!(
