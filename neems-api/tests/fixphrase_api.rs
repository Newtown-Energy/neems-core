...
async fn test_encode_fixphrase_success() {
    ...
    let response = client.get("/api/1/fixphrase/encode/42.3601/-71.0589").dispatch().await;
...
async fn test_encode_fixphrase_invalid_coords() {
    ...
    let response = client.get("/api/1/fixphrase/encode/91.0/0.0").dispatch().await;
    assert_eq!(response.status(), Status::BadRequest);
    let error: FixPhraseError = response.into_json().await.unwrap();
    assert!(matches!(error, FixPhraseError::InvalidLatitude));

    // Test invalid longitude
    let response = client.get("/api/1/fixphrase/encode/0.0/181.0").dispatch().await;
    let error: FixPhraseError = response.into_json().await.unwrap();
...
async fn test_api_response_structure() {
    let client = Client::tracked(test_rocket_no_db()).await.expect("valid rocket instance");
    let response = client.get("/api/1/fixphrase/encode/42.1409/-76.8518").dispatch().await;
