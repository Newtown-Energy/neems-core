...
async fn test_users_cannot_access_different_company_users() {
    ...
    // Check that we got some expected users from Test Company 1 (but don't require
    // all)
    let emails: Vec<&String> = users.iter().map(|u| &u.email).collect();
    assert!(
        emails.contains(&&"user@testcompany.com".to_string()),
        "Should contain user@testcompany.com"
    );
    assert!(
        emails.contains(&&"user@company1.com".to_string()),
        "Should contain user@company1.com"
    );
...
async fn test_nonexistent_company_users() {
    ...
    let response = client.get("/api/1/Companies/99999/Users").cookie(admin_cookie).dispatch().await;
...
async fn test_empty_users_for_existing_company() {
    ...
    // Get a pre-created test company (Test Company 2 has user@company2.com but for
    // this test we'll use a company without users) Actually, let's create a new
    // company for this specific test case since we need an empty one
