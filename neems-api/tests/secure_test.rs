...
async fn test_admin_only_endpoint_with_admin_user() {
    ...
    assert!(
        json_response["message"]
            .as_str()
            .unwrap()
            .contains("test_superadmin@example.com")
    );
}
...
async fn test_admin_and_staff_endpoint_with_both_roles() {
    ...
    let response = client
        .get("/api/1/test/admin-and-staff")
        .cookie(session_cookie)
        .dispatch()
        .await;
...
async fn test_no_admin_allowed_endpoint_with_admin() {
    ...
    let response = client
        .get("/api/1/test/no-admin-allowed")
        .cookie(session_cookie)
        .dispatch()
        .await;
...
async fn test_any_admin_or_staff_endpoint_with_admin() {
    ...
    let response = client
        .get("/api/1/test/any-admin-or-staff")
        .cookie(session_cookie)
        .dispatch()
        .await;
...
async fn test_newtown_admin_only_endpoint() {
    ...
    let response = client
        .get("/api/1/test/newtown-admin-only")
        .cookie(session_cookie)
        .dispatch()
        .await;
    ...
    assert!(
        json_response["message"]
            .as_str()
            .unwrap()
            .contains("newtown_superadmin@example.com")
    );
}
...
async fn test_newtown_staff_only_endpoint() {
    ...
    let response = client
        .get("/api/1/test/newtown-staff-only")
        .cookie(session_cookie)
        .dispatch()
        .await;
