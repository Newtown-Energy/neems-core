...
#[rocket::async_test]
async fn test_from_site_defaults_creates_charge_and_discharge_commands() {
    let client = Client::tracked(fast_test_rocket()).await.expect("valid rocket instance");
    let admin_cookie = login_admin(&client).await;

    // Persist the demo windows on Test Site 1 first — the helper reads them
    // from the row, so the wizard must save them before calling this.
    let site_patch = json!({
        "power_kw": 5000.0,
    ...
    let body = json!({
        "name": "Peak-season Defaults",
        "description": "Generated by wizard test",
        "end_of_charge_soc_percent": 100
    });
    let response = client
        .post("/api/1/Sites/1/ScheduleLibraryItems/FromSiteDefaults")
        .cookie(admin_cookie.clone())
        .json(&body)
        .dispatch()
        .await;
    ...
    assert_eq!(response.status(), Status::Created);
    let item: ScheduleLibraryItem = response.into_json().await.expect("valid JSON");
    assert_eq!(item.name, "Peak-season Defaults");
    assert_eq!(item.commands.len(), 2);

    let charge = item
        .commands
        .iter()
        .find(|c| matches!(c.command_type, CommandType::Charge))
        .expect("charge command");
    assert_eq!(charge.execution_offset_seconds, 0);
    assert_eq!(charge.duration_seconds, Some(8 * 3600));
    assert_eq!(charge.target_soc_percent, Some(100));

    let discharge = item
        .commands
        .iter()
        .find(|c| matches!(c.command_type, CommandType::Discharge))
        .expect("discharge command");
    assert_eq!(discharge.execution_offset_seconds, 16 * 3600);
    assert_eq!(discharge.duration_seconds, Some(4 * 3600));
    assert!(discharge.target_soc_percent.is_none());
}
