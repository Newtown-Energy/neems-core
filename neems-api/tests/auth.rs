...
async fn login_user(
    ...
    let response = client.post("/api/1/login").json(&login_body).dispatch().await;
...
async fn test_wrong_email() {
    let client = rocket::local::asynchronous::Client::tracked(fast_test_rocket()).await.unwrap();
...
async fn test_authenticated_user_has_roles() {
    ...
    // Use golden DB user with multiple roles (admin_staff@example.com has admin +
    // staff)
    let login_body = json!({
    ...
    let session_cookie =
        response.cookies().get("session").expect("Session cookie should be set").clone();
    ...
    assert!(login_roles.iter().any(|r| r.as_str() == Some("staff")));

    // Test that we can access protected routes
    let response = client.get("/api/1/hello").cookie(session_cookie).dispatch().await;
    assert_eq!(response.status(), Status::Ok);
    ...
    assert!(hello_roles.iter().any(|r| r.as_str() == Some("staff")));

    // Test role checking methods (we'll do this by examining the session guard
    // directly) This is a bit complex since we need to test the guard logic
    // For now, we verify that the authentication works with multiple roles
}

// test_role_helper_methods removed - no actual testing logic, just basic auth
// which is covered elsewhere

// COMPREHENSIVE AUTHENTICATION FLOW TESTS

// test_login_hello_data_consistency removed - basic functionality covered by
// test_complete_auth_flow

/// Test complete authentication flow: login → use session → logout → verify
/// session invalid
#[tokio::test]
async fn test_complete_auth_flow() {
    ...
    let response = client.get("/api/1/hello").cookie(session_cookie.clone()).dispatch().await;
    assert_eq!(response.status(), Status::Ok);
    ...
    // 4. Logout
    let logout_response =
        client.post("/api/1/logout").cookie(session_cookie.clone()).dispatch().await;
...
// test_user_without_roles_fails removed - just tests basic auth which is
// covered by test_complete_auth_flow
...
async fn test_user_role_assignment() {
    ...
    let session_cookie = response.cookies().get("session").expect("Session cookie should be set");
