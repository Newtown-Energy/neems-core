...
use neems_api::models::{Company, Role, Site, UserWithRoles};
...
async fn test_company_users_navigation() {
    ...
    let odata_response: serde_json::Value = companies_response.into_json().await.expect("valid OData JSON");
...
async fn test_navigation_requires_authentication() {
    ...
    for endpoint in endpoints {
        ...
        assert_eq!(response.status(), Status::Unauthorized, "Endpoint {} should require auth", endpoint);
    }
}
