...
async fn test_users_cannot_access_different_company_sites() {
    ...
    let response = client.get(&url).cookie(user_cookie.clone()).dispatch().await;
...
async fn test_newtown_admin_can_access_any_company_sites() {
    ...
    let url = format!("/api/1/Companies/{}/Sites", company1.id);
    let response = client.get(&url).cookie(newtown_admin_cookie.clone()).dispatch().await;
    ...
    let url = format!("/api/1/Companies/{}/Sites", company2.id);
    let response = client.get(&url).cookie(newtown_admin_cookie).dispatch().await;
...
async fn test_newtown_staff_can_access_any_company_sites() {
    ...
    let url = format!("/api/1/Companies/{}/Sites", company.id);
    let response = client.get(&url).cookie(newtown_staff_cookie).dispatch().await;
...
async fn test_nonexistent_company_sites() {
    ...
    let response = client.get("/api/1/Companies/99999/Sites").cookie(admin_cookie).dispatch().await;
