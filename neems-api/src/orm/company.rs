...
pub fn get_company_by_name(
    ...
    let result = companies.filter(name.eq(&comp.name)).first::<Company>(conn).optional()?;
    Ok(result)
}
...
pub fn insert_company(
    ...
    let new_comp = NewCompany { name: comp_name };

    diesel::insert_into(companies).values(&new_comp).execute(conn)?;

    let last_id = diesel::sql_query("SELECT last_insert_rowid() as last_insert_rowid")
        .get_result::<LastInsertRowId>(conn)?
        .last_insert_rowid;

    let company = companies.filter(id.eq(last_id as i32)).first::<Company>(conn)?;
...
pub fn get_company_by_id(
    ...
    use crate::schema::companies::dsl::*;
    let result = companies.filter(id.eq(company_id)).first::<Company>(conn).optional()?;
    Ok(result)
}
...
/// Returns Ok(true) if company was found and deleted, Ok(false) if not found,
/// Err on DB error.
pub fn delete_company(
    ...
    use crate::schema::companies::dsl::*;
    let company_to_delete =
        match companies.filter(id.eq(company_id)).first::<crate::models::Company>(conn) {
            Ok(company) => company,
    ...
    use crate::{models::NewDeletedCompany, schema::deleted_companies};
...
/// Gets company information for audit purposes, checking both active and
/// deleted companies.
///
...
///
/// # Returns
/// * `Ok(Some((name, is_deleted)))` - Company found with name and deletion
///   status
...
pub fn get_company_for_audit(
    ...
    if let Ok(company) = companies
        .filter(companies_id.eq(company_id))
        .first::<crate::models::Company>(conn)
    {
    ...
    if let Ok(deleted_company) = deleted_companies
        .filter(deleted_companies_id.eq(company_id))
        .first::<crate::models::DeletedCompany>(conn)
    {
...
mod tests {
    ...
    fn test_company_with_timestamps() {
        ...
        // Insert company
        let company =
            insert_company(&mut conn, "Timestamp Test Company".to_string(), None).unwrap();
