...
use crate::{
    admin_init_fairing::admin_init_fairing,
    orm::{
        DbConn, SiteDbConn, neems_data::set_foreign_keys_fairing as site_set_foreign_keys_fairing,
        run_migrations_fairing, set_foreign_keys_fairing,
    },
};

/// Creates a test rocket instance configured to use test-data.db for the site
/// database.
///
...
pub fn test_rocket_with_site_db() -> Rocket<Build> {
    // Configure the main API database (temporary file for isolation with unsafe
    // fast pragmas)
    let temp_db_path = std::env::temp_dir().join(format!("test_db_{}.sqlite", Uuid::new_v4()));
    let unique_db_name = format!(
        "sqlite://{}?synchronous=OFF&journal_mode=OFF&locking_mode=EXCLUSIVE&temp_store=MEMORY&cache_size=-64000",
    ...
    // Use SITE_DATABASE_URL to find the correct directory, then replace filename
    // with test-data.db
    let test_db_path = if let Ok(site_db_url) = std::env::var("SITE_DATABASE_URL") {
        ...
        std::env::current_dir().expect("current directory").join("test-data.db")
    };
    ...
    if !test_db_path.exists() {
        panic!(
            "Test database file not found at: {}. Please ensure test-data.db exists.",
            ...
            match SqliteConnection::establish(&format!("sqlite://{}", test_db_path.display())) {
                    ...
                    match diesel::sql_query("SELECT COUNT(*) FROM sources").execute(&mut conn) {
                        ...
                        Err(e) => {
                            ...
                            if error_msg.contains("database is locked") && retries < max_retries {
                                retries += 1;
                                println!(
                                    "Database locked during verification, retrying ({}/{})",
                                    retries, max_retries
                                );
                                sleep(Duration::from_millis(100 * retries as u64));
                            } else {
                                panic!(
                                    "Sources table not accessible in test database at {}: {}",
                                    test_db_path.display(),
                                    e
                                );
                            }
                ...
                Err(e) => {
                    ...
                    if error_msg.contains("database is locked") && retries < max_retries {
                        retries += 1;
                        println!(
                            "Database locked during connection, retrying ({}/{})",
                            retries, max_retries
                        );
                        sleep(Duration::from_millis(100 * retries as u64));
                    } else {
