...
impl DataAggregator {
    pub fn new(database_path: Option<&str>) -> Self {
        let database_path = match database_path {
            ...
            None => env::var("SITE_DATABASE_URL").unwrap_or_else(|_| "site-data.sqlite".to_string()),
        };
    ...
    fn collect_from_sources(connection: &mut SqliteConnection) -> Result<(), Box<dyn Error + Send + Sync>> {
    ...
    pub fn insert_reading(connection: &mut SqliteConnection, reading: NewReading) -> Result<(), Box<dyn Error + Send + Sync>> {
        use schema::readings;
    ...
    pub fn insert_readings_batch(connection: &mut SqliteConnection, readings: Vec<NewReading>) -> Result<(), Box<dyn Error + Send + Sync>> {
        use schema::readings;
...
pub fn create_source(connection: &mut SqliteConnection, new_source: NewSource) -> Result<Source, Box<dyn Error>> {
    use schema::sources;
...
pub fn get_source_by_name(connection: &mut SqliteConnection, source_name: &str) -> Result<Option<Source>, Box<dyn Error>> {
    use schema::sources::dsl::*;
...
/// Update a source
pub fn update_source(connection: &mut SqliteConnection, source_id: i32, updates: UpdateSource) -> Result<Source, Box<dyn Error>> {
    use schema::sources::dsl::*;
...
pub fn get_recent_readings(connection: &mut SqliteConnection, src_id: i32, limit: i64) -> Result<Vec<Reading>, Box<dyn Error>> {
...
pub fn read_aggregated_data(database_path: Option<&str>) -> Result<Vec<(Source, Vec<Reading>)>, Box<dyn Error>> {
