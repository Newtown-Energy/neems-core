//! Newtown's alarm definitions: the digital points from the client's alarm
//! matrix, with the severity levels we assign them. See
//! [`crate::rtac::alarm_definitions`] for the register layout and what the
//! levels mean.
//!
//! ## Register layout
//!
//! Where Newtown's alarms sit in the packed alarm block:
//!
...
//!
//! Hand-curated rather than generated; `every_definition_has_sld_meta` in
//! [`crate::rtac::design`] catches drift against the generated
//! [`super::alarm_sld_meta`] table.

use crate::rtac::alarm_definitions::{AlarmDefinition, AlarmZone};
...
/// Per-megapack alarm type names (30 types per megapack, matching DNP3 points
/// 1–30)
#[cfg(test)]
...
/// All alarm definitions for the Newtown site design.
///
/// Organized by zone in alarm-number order. Register indices and bit
/// positions correspond to the Modbus alarm register layout documented in
/// [`crate::rtac::alarm_definitions`].
...
mod tests {
    ...
    fn find(alarm_num: u16) -> &'static AlarmDefinition {
        ALARM_DEFINITIONS
            .iter()
    ...
    fn meta(alarm_num: u16) -> &'static AlarmSldMeta {
        super::super::alarm_sld_meta::ALARM_SLD_META
            .iter()
            .find(|m| m.alarm_num == alarm_num)
            .expect("alarm has SLD metadata")
    }

    fn in_zone(zone: AlarmZone) -> impl Iterator<Item = &'static AlarmDefinition> {
    ...
    fn test_known_sld_meta_values() {
        ...
        assert_eq!(meta(101).sld_targets, &["52-MAIN-1"]);
        // A megapack spare (622) has no operator message.
        assert_eq!(meta(622).message_opt(), None);
    }

    #[test]
    fn discrete_address_is_the_alarm_number() {
        // The client's addressing, pinned for a few spelled-out numbers. The
        // invariant over the whole table is checked for every design.
        assert_eq!(find(1).discrete_address(), 1); // site loss_fiber
        assert_eq!(find(ESTOP_ALARM_NUM).discrete_address(), 104);
        assert_eq!(find(FIRE_ALARM_NUM).discrete_address(), 401);
        assert_eq!(find(601).discrete_address(), 601); // MP-1A megapack_loss_of_comms
        assert_eq!(find(780).discrete_address(), 780); // MP-2C, last digital
    }
    ...
    fn test_estop_alarm_exists() {
        let estop = find(ESTOP_ALARM_NUM);
        assert_eq!(estop.name, "estop");
        assert_eq!(estop.level, 2);
    }

    #[test]
    fn test_fire_alarm_exists() {
        let fire = find(FIRE_ALARM_NUM);
        assert_eq!(fire.name, "fire_alarm");
        assert_eq!(fire.level, 1);
    }

    #[test]
    fn test_find_by_zone() {
        let site_alarms: Vec<_> = in_zone(AlarmZone::Site).collect();
