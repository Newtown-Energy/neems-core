...
This guide covers all API endpoints available in the neems-core system, including authentication, user management, role management, institution management, and utility endpoints.
...
Most endpoints require authentication via session cookies. Use the login endpoint to obtain a session cookie, then include `credentials: 'include'` in all subsequent requests.
