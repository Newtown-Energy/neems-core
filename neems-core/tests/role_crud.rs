...
async fn test_get_role_endpoint_success() {
    ...
    // Create a test role
    let created_role = create_test_role(
        &client,
        &admin_cookie,
        "Test Get Role",
        Some("A role for get testing"),
    )
    .await;
...
async fn test_update_role_endpoint_set_description_to_null() {
    ...
    let created_role =
        create_test_role(&client, &admin_cookie, "Test Role", Some("Has description")).await;
...
async fn test_delete_role_endpoint_success() {
    ...
    let get_response_after = client.get(&get_url).cookie(admin_cookie).dispatch().await;
