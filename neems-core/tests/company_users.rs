...
async fn test_users_cannot_access_different_company_users() {
    ...
    let _user1_company1 = create_user_with_role(&client, &admin_cookie, "admin@company1.com", company1.id, "admin").await;
    let _user1_company2 = create_user_with_role(&client, &admin_cookie, "admin@company2.com", company2.id, "admin").await;
...
async fn test_newtown_staff_can_access_any_company_users() {
    ...
    let _newtown_staff = create_user_with_role(&client, &admin_cookie, "newtownstaff@newtown.com", newtown_company.id, "newtown-staff").await;
...
async fn test_users_response_format() {
    ...
    let company = create_company(&client, &admin_cookie, "Test Company").await;
    let _user = create_user_with_role(&client, &admin_cookie, "user@testcompany.com", company.id, "admin").await;
...
async fn test_nonexistent_company_users() {
    ...
    let response = client.get("/api/1/company/99999/users")
        .cookie(admin_cookie)
        .dispatch()
        .await;
