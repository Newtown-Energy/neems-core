...
async fn test_delete_company() {
    ...
    // 1. Create a test company
    let new_comp = CompanyNoTime { name: "Company to Delete".to_string() };
    let create_response = client.post("/api/1/companies")
        .cookie(session_cookie.clone())
        .json(&new_comp)
        .dispatch()
    ...
    let created_company: Company = create_response.into_json().await.expect("valid JSON response");
    ...
    let list_response = client.get("/api/1/companies")
        .cookie(session_cookie.clone())
        .dispatch()
        .await;
    assert_eq!(list_response.status(), Status::Ok);
    
    let list: Vec<Company> = list_response.into_json().await.expect("valid JSON response");
...
async fn test_delete_nonexistent_company() {
    ...
    let delete_response = client.delete("/api/1/companies/99999")
        .cookie(session_cookie)
        .dispatch()
        .await;
