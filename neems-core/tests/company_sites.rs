...
async fn setup_sites_for_company(client: &Client, admin_cookie: &rocket::http::Cookie<'static>, company_id: i32, count: usize) -> Vec<Site> {
    ...
    for i in 0..count {
        ...
        let response = client.post("/api/1/sites")
            .cookie(admin_cookie.clone())
            .json(&new_site)
            .dispatch()
            .await;
...
async fn test_company_users_can_access_own_company_sites() {
    ...
    let (_email, password) = create_user_with_role(&client, &admin_cookie, "user@testcompany.com", company.id, "admin").await;
...
async fn test_users_cannot_access_different_company_sites() {
    ...
    let (_email, password) = create_user_with_role(&client, &admin_cookie, "user@company1.com", company1.id, "admin").await;
...
async fn test_newtown_admin_can_access_any_company_sites() {
    ...
    let (_email, password) = create_user_with_role(&client, &admin_cookie, "newtownadmin@newtown.com", newtown_company.id, "newtown-admin").await;
...
async fn test_newtown_staff_can_access_any_company_sites() {
    ...
    let (_email, password) = create_user_with_role(&client, &admin_cookie, "newtownstaff@newtown.com", newtown_company.id, "newtown-staff").await;
...
async fn test_nonexistent_company_sites() {
    ...
    let response = client.get("/api/1/company/99999/sites")
        .cookie(admin_cookie)
        .dispatch()
        .await;
...
async fn test_empty_sites_for_existing_company() {
    ...
    // Create user for this company
    let (_email, password) = create_user_with_role(&client, &admin_cookie, "user@empty.com", company.id, "admin").await;
