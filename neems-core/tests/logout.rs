...
async fn test_logout() {
    ...
    let logout_response = client.post("/api/1/logout")
    ...
    assert!(session_cookie_after_logout.is_none() || session_cookie_after_logout.unwrap().value().is_empty());
    
    // Test that accessing protected endpoint fails after logout
    let protected_response = client.get("/api/1/hello")
        .cookie(session_cookie.clone())
        .dispatch()
        .await;
