...
async fn test_site_endpoints_require_authentication() {
    ...
    let response = client.put("/api/1/sites/1")
        .json(&update_site)
        .dispatch()
        .await;
    assert_eq!(response.status(), Status::Unauthorized);
...
async fn test_company_admin_can_crud_own_company_sites() {
    ...
    let _company_admin = create_user_with_role(&client, &admin_cookie, "admin@testcompany.com", company.id, "admin").await;
    ...
    let response = client.get("/api/1/sites")
        .cookie(admin_session.clone())
        .dispatch()
        .await;
...
async fn test_company_admin_cannot_access_different_company_sites() {
    ...
    let _company1_admin = create_user_with_role(&client, &admin_cookie, "admin1@company1.com", company1.id, "admin").await;
