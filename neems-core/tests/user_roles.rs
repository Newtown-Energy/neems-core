...
use neems_core::models::{User, UserNoTime, Company, CompanyNoTime, Role};
...
async fn setup_test_data(client: &Client) -> (TestUsers, TestCompanies) {
    ...
    let (users, companies) = db_conn.run(|conn| {
        ...
        let other_company_user = insert_user(conn, UserNoTime {
...
async fn test_get_user_roles_user_cannot_view_others_roles() {
    ...
    let response = client.get(format!("/api/1/users/{}/roles", users.other_company_user.id))
        .cookie(session_cookie)
        .dispatch()
        .await;
